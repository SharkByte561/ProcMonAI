"""
Interactive ProcmonAI agent with SQL-powered analysis.

Workflow:
1. Capture or load PML file
2. Convert PML to CSV
3. Chat using natural language → SQL queries via DuckDB
"""
...
# Try to import SQL chat modules
try:
    from sql_chat import SQLChat, interactive_sql_chat
    SQL_CHAT_AVAILABLE = True
except ImportError as e:
    print(f"[DEBUG] SQL chat import failed: {e}")
    SQLChat = None  # type: ignore
    interactive_sql_chat = None  # type: ignore
    SQL_CHAT_AVAILABLE = False

# Fallback to CSV chat if SQL chat unavailable
try:
    from csv_chat import CSVChat, interactive_chat as csv_interactive_chat
    CSV_CHAT_AVAILABLE = True
except ImportError:
    CSVChat = None  # type: ignore
    csv_interactive_chat = None  # type: ignore
...
def interactive_loop() -> None:
    """Main interactive loop with SQL-powered chat."""
    print("=" * 70)
    print(" ProcmonAI - SQL-Powered Analysis")
    print("=" * 70)
    print("Commands:")
    ...
    print("  chat    - Chat with AI using SQL queries (NEW!)")
    print("  report  - Generate Excel report")
    print("  load    - Load an existing CSV or PML file")
    print("  quit    - Exit")
    print("=" * 70)

    if SQL_CHAT_AVAILABLE:
        print("\n[OK] SQL chat available (NL → SQL → DuckDB)")
    elif CSV_CHAT_AVAILABLE:
        print("\n[FALLBACK] Using CSV chat (SQL chat unavailable)")
    else:
    ...
    while True:
        ...
        if cmd in ("quit", "exit", "q"):
            ...
            try:
                ...
                print("Tip: Use 'chat' to ask questions with SQL!")
            ...
            if path.suffix.lower() == '.csv':
                ...
                print("Use 'chat' to analyze with SQL queries.")
            ...
            if not SQL_CHAT_AVAILABLE and not CSV_CHAT_AVAILABLE:
            ...
            # Start SQL chat (preferred) or fallback to CSV chat
            if SQL_CHAT_AVAILABLE:
                interactive_sql_chat(str(last_csv))
            elif CSV_CHAT_AVAILABLE:
                csv_interactive_chat(str(last_csv))
            ...
            print("  chat    - Chat with AI using SQL queries")
